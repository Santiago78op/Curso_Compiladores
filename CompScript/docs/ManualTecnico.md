...
### Decisiones y correcciones de la auditoría cruzada (2026-07-24)

Una segunda auditoría, esta vez cruzada sobre varios proyectos del curso, dejó estas correcciones y decisiones documentadas:

10. **Una lista dinámica se declara siempre vacía** y se llena con `push()`: un `init` en su declaración (`let l: list<int> = [1,2];`) antes se descartaba en silencio y ahora es error semántico explícito (`A.Declaracion.ejecutar`, caso `LIST`). Es coherente con el diseño de las listas como estructura que crece dinámicamente, y se distingue de los vectores, que sí admiten inicialización por literal.
11. **Semántica de referencia de vectores/listas/structs — decisión documentada.** Declarar con un literal *materializa una estructura nueva* (`validarVector` construye una lista propia mientras valida tipos), pero una asignación posterior de variable a variable *comparte la referencia* (`b = a;` deja a `b` y `a` sobre la misma estructura, como las referencias a objeto de Java). El enunciado no fija una semántica de referencia; se adopta esta y se documenta explícitamente para que no sea una sorpresa: si se necesita una copia independiente, se construye un literal nuevo.
12. **El acceso a campos de struct es de un solo nivel** (`struct.campo`), exactamente como lo define y ejemplifica el enunciado (§5.20.3, `e1.edad`). Un campo cuyo tipo es a su vez un struct se puede declarar, leer o reemplazar completo (`p.domicilio`), pero el acceso encadenado `p.domicilio.ciudad` no forma parte del lenguaje: el enunciado no lo exige y la gramática de acceso/asignación es deliberadamente de un nivel.
13. **`round` acepta `char`** tratándolo por su código ASCII (`round('a')` = 97). El enunciado (§5.25) pide "un valor numérico" sin excluir `char`, y CompScript trata `char` como numérico vía ASCII de forma uniforme en sus tablas de operadores; aceptarlo aquí es consistente con ese modelo.
14. **El orden de evaluación de los argumentos sigue el orden de los parámetros declarados**, no el orden textual de la llamada (los argumentos se asocian por identificador, decisión 5). Solo es observable si dos argumentos tienen efectos secundarios (por ejemplo, llamadas con `push`/`pop`); se documenta por completitud.
