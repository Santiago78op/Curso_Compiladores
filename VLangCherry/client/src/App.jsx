...
import EJEMPLO_INICIAL from './ejemplo-inicial.js';
import './App.css';
...
export default function App() {
  const [archivos, setArchivos] = useState(() => [
    archivoNuevo('principal.vch', EJEMPLO_INICIAL),
  ]);
  ...
  const manejarNuevo = () => {
    const a = archivoNuevo('sin-titulo-' + (archivos.length + 1) + '.vch', '');
  ...
  const manejarGuardar = () => {
    ...
    enlace.href = url;
    enlace.download = archivoActivo.nombre.endsWith('.vch')
      ? archivoActivo.nombre
      : archivoActivo.nombre + '.vch';
    enlace.click();
