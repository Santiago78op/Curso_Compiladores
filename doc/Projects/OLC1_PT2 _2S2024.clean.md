...
> Nota de conversión: la numeración de subsecciones que sigue es **fiel al PDF original** (se repite igual en esta Tabla de Contenido y en los encabezados del cuerpo del documento), no es un error introducido por la conversión a Markdown: falta el `4.3`, `5.5.6` se usa dos veces (Raíz y Módulo), `5.16.2.2` se usa dos veces (Case y Default), `5.21` se usa dos veces (Llamadas y Función echo) y la numeración salta de `5.23` a `5.25` en las funciones nativas.
...
Por tanto, a usted, que es estudiante del curso de Compiladores 1, se le encomienda realizar el proyecto llamado CompInterpreter, dado sus altos conocimientos en temas de análisis léxico, sintáctico y semántico.

**Arquitectura Cliente - Servidor**

<!-- revisar: dato incierto — aquí el PDF original probablemente incluye un diagrama/imagen de la arquitectura cliente-servidor; no es recuperable como texto sin poppler/pdftoppm. -->
...
_Figura 1: Propuesta de Interfaz_

<!-- revisar: dato incierto — la Figura 1 es una imagen en el PDF original, no reproducible en Markdown sin poppler/pdftoppm. -->
...
| Carácter | char | Tipo de dato que únicamente aceptará un único carácter, estará delimitado por comillas simples | 'a', 'b', 'c', etc | Se permitirá cualquier carácter entre las comillas simples, incluyendo las secuencias de escape | ' ' (carácter 0) |
...
<!-- Observación de la conversión: en esta tabla no aparece fila/columna Boolean ni Cadena — a diferencia de Suma y Resta, el operador * no está definido para Boolean ni para Cadena en el texto extraído del PDF. -->
...
<!-- Observación de la conversión: la división siempre produce Decimal (división real, no entera), tampoco está definida para Boolean ni Cadena. -->

#### 5.5.5 Potencia

Es una operación aritmética de la forma a^b, donde a es el valor de la base y b es el valor del exponente que nos indicará cuantas veces queremos multiplicar el mismo número. Por ejemplo 5 ^ 3, a=5 y b=3 tendríamos que multiplicar 3 veces 5 para obtener el resultado final; 5x5x5 que da como resultado 125.
...
Es una operación aritmética de la forma a $ b, donde a es el valor de la base y b es el valor del índice de la raíz. Por ejemplo 5 $ 3, a = 5 y b=3 representa la raíz cúbica de 5.
...
<!-- revisar: dato incierto — el texto extraído del PDF da Entero % Entero = Decimal (idéntico a la tabla de Raíz). Es inusual que el módulo de dos enteros no dé Entero; podría ser el comportamiento real definido por el enunciado o un error del documento original. Verificar contra el PDF visual si es posible. -->
...
| --- | --- | --- |
| `==` | Igualación: compara ambos valores y verifica si son iguales.<br>- Iguales → True<br>- No iguales → False | `1==1`<br>`"hola" == "hola"`<br>`25.654 == 54.34` |
| `!=` | Diferenciación: compara ambos lados y verifica si son distintos.<br>- Iguales → False<br>- No iguales → True | `1 != 2`<br>`var1 != var2`<br>`50 != 30` |
| `<` | Menor que: compara ambos lados y verifica si el derecho es mayor que el izquierdo.<br>- Derecho mayor → True<br>- Izquierdo mayor → False | `25.5 < 30`<br>`54 < 25`<br>`50 < 'F'` |
| `<=` | Menor o igual que: Compara ambos lados y verifica si el derecho es mayor o igual que el izquierdo.<br>- Derecho mayor o igual → True<br>- Izquierdo mayor → False | `25.5 <= 30`<br>`54 <= 25`<br>`50 <= 'F'` |
| `>` | Mayor que: compara ambos lados y verifica si el izquierdo es mayor que el derecho.<br>- Derecho mayor → False<br>- Izquierdo mayor → True | `25.5 > 30`<br>`54 > 25`<br>`50 > 'F'` |
| `>=` | Mayor o igual que: Compara ambos lados y verifica si el izquierdo es mayor o igual que el derecho.<br>- Derecho mayor → False<br>- Izquierdo mayor o igual → True | `25.5 >= 30`<br>`54 >= 25`<br>`50 >= 'F'` |
...
<!-- revisar: dato incierto — el PDF original define esta matriz de compatibilidad para los operadores relacionales (encabezados de fila/columna: Entero, Decimal, Boolean, Carácter, Cadena, Nulo), pero el contenido de las celdas se perdió por completo en la conversión a texto (probablemente eran marcas/íconos, no texto). No se puede reconstruir sin abrir el PDF visualmente (falta poppler/pdftoppm en esta máquina). -->
...
**Sintaxis exacta confirmada** (verificada también contra el ejemplo de archivo de entrada del Anexo 11.1): el operador ternario usa la palabra reservada `if` seguida de la condición entre paréntesis, luego la expresión "verdadera", dos puntos `:`, y la expresión "falsa" — sin `else` y sin `?`. Se usa como expresión, asignable directamente a una variable: `resultado = if (suma > 15) "Mayor a 15" : "Menor o igual a 15";`
...
| --- | --- | --- | --- |
| \|\| | OR: compara expresiones lógicas y si al menos una es verdadera, entonces devuelve verdadero y en otro caso retorna falso. | `bandera \|\| 5<2` | Devuelve true si bandera es true |
| `&&` | AND: compara expresiones lógicas y si ambas son verdaderas, entonces devuelve verdadero y en otro caso retorna falso. | `flag && flag` | Devuelve true si flag es true |
| `!` | NOT: devuelve el valor inverso de una expresión lógica si esta es verdadera entonces devolverá false, de lo contrario retorna verdadero. | `!var` | Devuelve false si var es true |
...
**Comportamiento de fall-through confirmado literalmente**: el enunciado indica de forma explícita que si un `case` no tiene `break`, al terminar sus instrucciones el lenguaje **continúa evaluando los demás case** (fall-through estilo C/Java clásico) — no hay salto automático fuera del switch al final de cada case.
...
Observaciones:
- Si el parámetro tiene valor por defecto en la llamada puede omitirse en la llamada o bien se puede sobreescribir su valor.
...
- Se podrán llamar métodos y funciones antes que se encuentren declaradas, para ello se recomienda realizar 2 pasadas del AST generado: La primera para almacenar todas las funciones, y la segunda para las variables globales y la función ejecutar.
...
<!-- Nota de conversión: el "10" que sigue a "hola Mundo" en el bloque de arriba es la SALIDA esperada del ejemplo (echo variable1, donde variable1 vale 10), no un número de página suelto. El linter lo señaló como línea sospechosa, pero es contenido legítimo del ejemplo y se conserva. -->
...
Los reportes son parte fundamental de CompInterpreter, ya que muestra de forma visual las herramientas utilizadas para realizar la ejecución del código. A continuación, se muestran ejemplos de estos reportes. Queda a discreción del estudiante el diseño de estos, solo se pide que sean totalmente legibles.
